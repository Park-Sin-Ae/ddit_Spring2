...
@Data
public class PaginationInfoVO<T> {   // Generic 타입을 지정해버리면 안되는 이유 : 공지사항 게시판이면 noticeVO, 일반게시판이면 boardVO여야 하므로.
   ...
   public void setCurrentPage(int currentPage) {   // 현재 페이지를 기반으로 공식화
   ...
   public String getPagingHTML() {
      ...
      // 반복문 내 조건은 총 페이지가 있고 현재 페이지에 따라서 endPage 값이 결정됩니다.
      ...
      for(int i = startPage; i <= (endPage < totalPage ? endPage : totalPage); i++) {
         if(i == currentPage) {   // 현재페이지라면
            // 번호만 띄워주면 됨 + active 요소를 주면됨
            html.append("<li class='page-item active'><span class='page-link'>" + i +
                  "</span></li>");   
         } else {   // 현재 페이지가 아니므로 링크가 눌릴 수  있도록 a 태그를 줘야함
            html.append("<li class='page-item'><a href='' class='page-link' data-page='"+
                  i + "'>" + i + "</a></li>");
         }
