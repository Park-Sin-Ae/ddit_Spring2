...
public class ImageUpload {
   ...
   public String imageUpload(
      ...
      JsonObject json = new JsonObject(); // JSON 객체를 만들기 위한 준비, GSON라이브러리 필요
      PrintWriter printWriter = null;      // 외부 응답으로 내보낼 때 사용할 객체
      OutputStream out = null;         // 본문 내용에 추가한 이미지를 파일로 생성할 객체
      long limitSize = 1024 * 1024 * 2;   // 업로드 파일 최대 크기 (2MB 제한)
      
      // CKEditor 본문 내용에 이미지르 ㄹ업로드 해보면 'upload'라는 키로 파일 데이터가 전달되는걸 확인할 수 있따.
      MultipartFile file =  multiFile.getFile("upload");
      
      // 파일 객체가 null이 아니고 파일 사이즈가 0보다 크고 파일명이 공백이 아닌 경우에는
      // 무조건 파일 데이터가 존재하는 경우임
         ...
         if(file.getContentType().toLowerCase().startsWith("image/")) {
            if(file.getSize() > limitSize) {   // 업로드 한 파일 사이즈가 최대 크기보다 클 때
               ...
               try {
                  ...
                  // 폴더 구조가 존재하지 않는 경우 생성하고 존재하지 않는 경우 건너뜀.
