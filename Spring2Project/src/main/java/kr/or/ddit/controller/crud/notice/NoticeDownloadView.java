...
// AbstractView 클래스를 상속받아 renderMergedOutputModel 메소드를 재정의 하여 사용하면
...
public class NoticeDownloadView extends AbstractView {
	...
	protected void renderMergedOutputModel(
		...
		// User-Agent : 브라우저의 정보 DB 등 가져올 수있다.
		// 요청 Header 정보들 중, User-Agent 영역 안에 여러 키워드 정보들을 가지고
		// 특정 키워드가 포함되어 있는지를 체크해서 파일명의 출력 인코딩 부분을 설정한다.
		// 사용 브라우저 또는 현상에 따라 발생하는 알고리즘이므로, 내가 사용하게 되는 브라우저의 버전이나 얻어온
		// Header 정보들의 값에 따라 차이가 발생할 수 있다.
		String agent = request.getHeader("User-Agent");
		if(StringUtils.containsIgnoreCase(agent, "msie") ||
				StringUtils.containsIgnoreCase(agent, "trident")) {	// IE, Chrome 을 실행했을 때 해당 if 문이 돌게 한다.
			fileName = URLEncoder.encode(fileName, "UTF-8");
		}else {		// firefox, chrome 을 쓰는 사용자일 때
